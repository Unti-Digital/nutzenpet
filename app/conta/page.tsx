  ...
  return (
    <main className="min-h-screen bg-[#fffef9]">
      ...
      <section className="relative overflow-hidden px-5 py-14 sm:px-8 sm:py-20"><FloatingMotifs className="opacity-35" /><div className="relative mx-auto grid max-w-[1080px] overflow-hidden rounded-lg bg-white shadow-[0_22px_65px_rgba(18,63,85,.1)] lg:grid-cols-[0.86fr_1.14fr]">
        ...
        <form onSubmit={handleSubmit} className="reveal-up p-8 sm:p-10"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#67952F]">Bem-vindo de volta</p><h2 className="mt-3 text-3xl font-black text-[#123F55]">Acesse sua conta</h2><p className="mt-2 text-sm leading-6 text-slate-500">Consulte pedidos e atualize seus dados de entrega.</p><label className="mt-7 grid gap-2 text-xs font-bold text-slate-600">E-mail<input type="email" required className={fieldClass} /></label><label className="mt-4 grid gap-2 text-xs font-bold text-slate-600">Senha<input type="password" required className={fieldClass} /></label><div className="mt-3 flex justify-end"><button type="button" className="text-xs font-bold text-[#124D55] transition-colors duration-300 hover:text-[#FE8C05]">Esqueci minha senha</button></div><button type="submit" className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#FE8C05] text-sm font-black text-white transition-all duration-300 hover:scale-[0.98] hover:bg-[#CC632B] active:scale-95"><LogIn className="h-4 w-4" /> Entrar</button><div className="my-6 flex items-center gap-3 text-[10px] uppercase tracking-[0.16em] text-slate-400"><span className="h-px flex-1 bg-slate-200" /> ou <span className="h-px flex-1 bg-slate-200" /></div><button type="button" className="flex h-12 w-full items-center justify-center gap-2 rounded-full border-2 border-[#124D55] text-sm font-black text-[#124D55] transition-all duration-300 hover:scale-[0.98] hover:bg-[#F1F6E7] active:scale-95"><UserPlus className="h-4 w-4" /> Criar nova conta</button><Link href="/minha-conta" className="mt-5 flex items-center justify-center text-xs font-black text-[#124D55] underline decoration-[#B9DC80] decoration-2 underline-offset-4 transition-colors duration-300 hover:text-[#FE8C05]">Visualizar área da conta</Link><p className="mt-5 flex items-center justify-center gap-2 text-[10px] text-slate-400"><LockKeyhole className="h-3.5 w-3.5" /> Ambiente protegido</p></form>
      </div></section>
      <SiteFooter />
    </main>
  );
