...
type SubscriptionData = { id: number; plan_id: number; product_id: number; product_name?: string; product_slug?: string; product_image?: string; product_weight?: string; plan_name?: string; quantity: number; interval_value: number; interval_unit: string; discount_type: string; discount_value: number; status: string; next_charge_date: string | null; currency?: string; unit_price?: number; subscription_total?: number; available_plans?: Array<{ id: number; name: string }> };
type AffiliateData = { code: string; status: string; link: string; balances: Array<{ status: string; amount: string }>; sales_count: number; commission_count: number; referred_total: number; attribution_days: number; available_balance: number; minimum_withdrawal: number };
...
type WithdrawalData = { id: number; ticket_number: string; amount: string; status: string; note: string; created_at: string; updated_at: string };
...
export default function MyAccountPage() {
  ...
  async function loadAccount() {
    ...
    setUser(me.user);
    const [orderPayload, subscriptionPayload, affiliatePayload, addressPayload] = await Promise.all([
  ...
  async function subscriptionAction(id: number, action: "pause" | "cancel" | "reactivate" | "frequency", planId?: number) {
    const response = await fetch(`/api/subscriptions/${id}/${action}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: action === "frequency" ? JSON.stringify({ plan_id: planId }) : undefined });
  ...
  async function requestWithdrawal(amount: number) {
    const response = await fetch("/api/auth/withdrawals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount }) });
    const payload = await response.json().catch(() => null) as { ticket_number?: string; message?: string } | null;
    setMessage(response.ok ? `Solicitação ${payload?.ticket_number ?? ""} aberta para análise.` : payload?.message ?? "Não foi possível solicitar o saque.");
      ...
      <section className="px-5 py-10 sm:px-8 sm:py-14">
        <div className="mx-auto grid max-w-[1180px] gap-8 lg:grid-cols-[230px_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
            ...
            {activeTab === "afiliados" && affiliate && <AffiliatePanel affiliate={affiliate} commissions={commissions} withdrawals={withdrawals} onMessage={setMessage} onWithdrawal={requestWithdrawal} />}
...
function AffiliatePanel({ affiliate, commissions, withdrawals, onMessage, onWithdrawal }: { affiliate: AffiliateData; commissions: CommissionData[]; withdrawals: WithdrawalData[]; onMessage: (message: string) => void; onWithdrawal: (amount: number) => Promise<void> }) {
  const [withdrawalAmount, setWithdrawalAmount] = useState("");
  if (affiliate.status === "approved") {
    return <section className="reveal-up overflow-hidden rounded-lg bg-[#3E1255] p-7 text-white sm:p-10"><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#D9C7E3]">Programa de afiliados</p><h2 className="mt-3 text-3xl font-black">Seu painel agora tem um espaço exclusivo.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/70">Crie links por produto e campanha, consulte cliques, pedidos, comissões, PIX e solicitações de saque sem misturar esses dados com sua conta de cliente.</p><Link href="/painel-afiliado" className="group mt-7 inline-flex h-12 items-center gap-3 rounded-full bg-[#FE8C05] px-6 text-sm font-black text-white">Abrir painel de afiliado <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-2" /></Link></section>;
  }
  const balance = (status: string) => Number(affiliate.balances.find((item) => item.status === status)?.amount ?? 0);
  const approved = Number(affiliate.available_balance ?? balance("approved"));
  const pending = balance("pending");
  const minimumWithdrawal = Math.max(100, Number(affiliate.minimum_withdrawal ?? 100));
  const openWithdrawal = withdrawals.find((withdrawal) => ["requested", "in_review"].includes(withdrawal.status));
  const requestedAmount = Number(withdrawalAmount.replace(",", "."));
  const canRequestWithdrawal = !openWithdrawal && requestedAmount >= minimumWithdrawal && requestedAmount <= approved;
  ...
  return (
    <section className="reveal-up overflow-hidden rounded-lg border border-[#D9C7E3] bg-white">
      <div className="relative overflow-hidden bg-[#3E1255] p-7 text-white sm:p-10">
        <FloatingMotifs className="text-white opacity-10" />
        <div className="relative flex flex-wrap items-end justify-between gap-6">
          <div><div className="flex flex-wrap items-center gap-3"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#D9C7E3]">Programa de afiliados</p><span className="rounded-full bg-[#FE8C05] px-3 py-1 text-[9px] font-black uppercase">Conta ativa</span></div><h2 className="mt-4 text-3xl font-black">Seu painel de indicações</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-white/65">Compartilhe a NutzenPet e acompanhe as comissões geradas pelas compras atribuídas ao seu código.</p></div>
          <Link href="/afiliados" className="flex items-center gap-2 text-xs font-black text-[#FFD39C]">Regras do programa <ExternalLink className="h-4 w-4" /></Link>
        </div>
      </div>
      <div className="grid border-b border-[#E2D4E9] sm:grid-cols-2 xl:grid-cols-4"><AffiliateMetric icon={Banknote} label="Comissão disponível" value={formatMoney(approved)} detail="Aprovada para recebimento" /><AffiliateMetric icon={Clock3} label="Comissão pendente" value={formatMoney(pending)} detail="Aguardando aprovação" /><AffiliateMetric icon={ShoppingBag} label="Vendas indicadas" value={String(affiliate.sales_count)} detail="Pedidos atribuídos" /><AffiliateMetric icon={ReceiptText} label="Comissões geradas" value={String(affiliate.commission_count)} detail={formatMoney(affiliate.referred_total) + " em vendas"} /></div>
      <div className="grid lg:grid-cols-[minmax(0,1.65fr)_minmax(280px,.75fr)]">
        <div className="p-6 sm:p-8"><div className="flex items-center gap-3"><Link2 className="h-5 w-5 text-[#3E1255]" /><h3 className="text-xl font-black text-[#123F55]">Meu link de afiliado</h3></div><p className="mt-3 text-sm leading-6 text-slate-500">Compras iniciadas por este link ficam atribuídas à sua conta por {affiliate.attribution_days} dias neste navegador.</p><div className="mt-5 flex flex-col gap-3 sm:flex-row"><input value={affiliate.link} readOnly aria-label="Link de afiliado" className="h-12 min-w-0 flex-1 rounded-md border border-slate-200 bg-slate-50 px-4 text-sm font-bold text-slate-600" /><button type="button" onClick={() => void copyLink()} className="flex h-12 items-center justify-center gap-2 rounded-md bg-[#3E1255] px-6 text-xs font-black text-white"><Copy className="h-4 w-4" />Copiar link</button></div><div className="mt-5 flex flex-wrap gap-x-8 gap-y-2 text-xs text-slate-500"><span><strong className="text-[#123F55]">Código:</strong> {affiliate.code}</span><span><strong className="text-[#123F55]">Atribuição:</strong> {affiliate.attribution_days} dias</span><span><strong className="text-[#123F55]">Comissão:</strong> definida por produto</span></div></div>
        <aside className="border-t border-[#E2D4E9] bg-[#F5EFF8] p-6 sm:p-8 lg:border-l lg:border-t-0"><div className="flex items-center gap-3"><Banknote className="h-5 w-5 text-[#3E1255]" /><h3 className="text-xl font-black text-[#123F55]">Recebimentos</h3></div><p className="mt-6 text-xs text-slate-500">Saldo disponível</p><strong className="mt-1 block text-3xl font-black text-[#3E1255]">{formatMoney(approved)}</strong><label className="mt-5 grid gap-2 text-xs font-bold text-slate-600">Valor do saque<input type="number" min={minimumWithdrawal} max={approved || undefined} step="0.01" value={withdrawalAmount} onChange={(event) => setWithdrawalAmount(event.target.value)} className="h-11 rounded-md border border-[#D9C7E3] bg-white px-4 text-sm outline-none focus:border-[#3E1255]" /></label><button type="button" disabled={!canRequestWithdrawal} onClick={() => void onWithdrawal(requestedAmount)} className="mt-4 h-11 w-full rounded-full bg-[#FE8C05] text-xs font-black text-white transition-colors hover:bg-[#CC632B] disabled:cursor-not-allowed disabled:opacity-45">{openWithdrawal ? "Chamado em análise" : "Solicitar saque"}</button><p className="mt-4 text-xs leading-5 text-slate-500">Saque mínimo de {formatMoney(minimumWithdrawal)}. A solicitação abre um chamado para conferência da equipe.</p>{openWithdrawal && <p className="mt-3 rounded-md bg-white p-3 text-xs font-bold text-[#3E1255]">Ticket {openWithdrawal.ticket_number} · {formatMoney(openWithdrawal.amount)}</p>}</aside>
      </div>
      <div className="border-t border-[#E2D4E9] p-6 sm:p-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#3E1255]">Movimentações</p><h3 className="mt-2 text-xl font-black text-[#123F55]">Comissões recentes</h3></div><span className="text-xs font-bold text-slate-400">Dados reais do WooCommerce</span></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead><tr className="border-b border-slate-200 text-[10px] uppercase tracking-[0.14em] text-slate-400"><th className="pb-3">Pedido</th><th className="pb-3">Data</th><th className="pb-3">Origem</th><th className="pb-3">Comissão</th><th className="pb-3 text-right">Status</th></tr></thead><tbody>{commissions.length > 0 ? commissions.map((commission) => <tr key={commission.id} className="border-b border-slate-100 last:border-0"><td className="py-4 font-black text-[#123F55]">#{commission.order_number}</td><td className="py-4 text-slate-500">{new Date(commission.created_at.replace(" ", "T") + "Z").toLocaleDateString("pt-BR")}</td><td className="py-4"><strong className="block text-slate-600">{commission.source}</strong><span className="text-xs text-slate-400">{commission.product_name}</span></td><td className="py-4 font-black text-[#3E1255]">{formatMoney(commission.commission_amount)}</td><td className="py-4 text-right"><CommissionStatus status={commission.status} /></td></tr>) : <tr><td colSpan={5} className="py-10 text-center text-sm text-slate-400">Nenhuma comissão registrada até o momento.</td></tr>}</tbody></table></div></div>
      {withdrawals.length > 0 && <div className="border-t border-[#E2D4E9] bg-[#fffef9] p-6 sm:p-8"><h3 className="text-lg font-black text-[#123F55]">Chamados de saque</h3><div className="mt-4 grid gap-3">{withdrawals.map((withdrawal) => <div key={withdrawal.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-slate-200 bg-white p-4 text-sm"><div><strong className="text-[#3E1255]">{withdrawal.ticket_number}</strong><p className="mt-1 text-xs text-slate-400">Aberto em {new Date(withdrawal.created_at.replace(" ", "T") + "Z").toLocaleDateString("pt-BR")}</p></div><strong className="text-[#123F55]">{formatMoney(withdrawal.amount)}</strong><WithdrawalStatus status={withdrawal.status} /></div>)}</div></div>}
    </section>
  );
}

function WithdrawalStatus({ status }: { status: string }) {
  const labels: Record<string, string> = { requested: "Solicitado", in_review: "Em análise", paid: "Pago", rejected: "Rejeitado", cancelled: "Cancelado" };
  const styles: Record<string, string> = { requested: "bg-orange-50 text-[#CC632B]", in_review: "bg-[#F5EFF8] text-[#3E1255]", paid: "bg-green-50 text-green-700", rejected: "bg-red-50 text-red-700", cancelled: "bg-slate-100 text-slate-500" };
  return <span className={`rounded-full px-3 py-1 text-[9px] font-black uppercase ${styles[status] ?? "bg-slate-100 text-slate-500"}`}>{labels[status] ?? status}</span>;
}
