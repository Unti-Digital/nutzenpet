  ...
  return (
    <footer className="bg-[#3E1255] pt-12 text-white">
      <div className="mx-auto grid max-w-[1340px] gap-9 px-5 sm:grid-cols-2 sm:px-8 lg:grid-cols-[1.35fr_0.85fr_1.15fr_0.85fr_1.15fr]">
        ...
        <div><h3 className="text-sm font-black">Institucional</h3><nav className="mt-4 grid gap-2 text-xs text-white/70"><Link href="/sobre" className={footerLink}>Sobre nós</Link><Link href="/blog" className={footerLink}>Blog</Link><Link href="/nutzen-club" className={footerLink}>Nutzen Club</Link><Link href="/afiliados" className={footerLink}>Afiliados</Link><Link href="/seja-um-lojista-parceiro" className={footerLink}>Seja um lojista parceiro</Link><Link href="/conta" className={footerLink}>Minha conta</Link><Link href="/politica-de-privacidade" className={footerLink}>Política de privacidade</Link></nav></div>
        <div><h3 className="text-sm font-black">Produtos</h3><nav className="mt-4 grid gap-2 text-xs text-white/70">{featuredProducts.map((product) => <Link key={product.slug} href={`/produto/${product.slug}`} className={footerLink}>{product.shortName}</Link>)}<Link href="/produto" className="mt-1 font-black text-[#D9C7E3] transition-colors duration-300 hover:text-[#FE8C05]">Ver todos</Link></nav></div>
