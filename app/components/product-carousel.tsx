...
export function ProductCarousel({ products, startIndex = 0, className = "" }: { products: Product[]; startIndex?: number; className?: string }) {
  ...
  const desktopCarousel = totalItems > 3;
  const singleProduct = totalItems === 1;
  const twoProducts = totalItems === 2;

  function getItemsPerView() {
    if (window.innerWidth >= 1024) return desktopCarousel ? 3 : totalItems;
    if (window.innerWidth >= 640) return desktopCarousel ? 2 : totalItems;
    return 1;
  }
  ...
  const trackLayout = desktopCarousel
    ...
    : `flex snap-x snap-mandatory overflow-x-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:overflow-visible ${singleProduct ? "sm:grid-cols-1" : twoProducts ? "sm:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3"}`;
  const itemLayout = desktopCarousel
    ? "min-w-[86%] snap-start sm:min-w-[calc(50%-0.75rem)] lg:min-w-[calc(33.333%-1rem)]"
    : "min-w-[86%] snap-start sm:min-w-0";

  return (
    <div className={`${singleProduct ? "sm:mx-auto sm:max-w-[430px]" : twoProducts ? "sm:mx-auto sm:max-w-[900px]" : ""} ${className}`}>
      {totalItems > 1 && <div className={`mb-5 flex w-full items-center justify-between md:justify-end md:gap-2 ${desktopCarousel ? "md:flex" : "md:hidden"}`} aria-label="Controles do carrossel de produtos">
